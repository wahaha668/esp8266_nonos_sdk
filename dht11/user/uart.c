...
////    if(pRxBuff->BuffState == EMPTY){
////    	system_os_post(uart_recvTaskPrio, 1, 0);
////    }else{
////    	system_os_post(uart_recvTaskPrio, 2, 0);
////    }
////    if(pRxBuff->pRcvMsgBuff == "sdf"){
////    	system_os_post(uart_recvTaskPrio, 3, 0);
////    }
...
//    WRITE_PERI_REG(UART_INT_CLR(uart_no), UART_RXFIFO_FULL_INT_CLR);
	  ...
	  RcvMsgBuff *pRxBuff = (RcvMsgBuff *)para;
	  //INFO("uart is coming! \r\n");
	  //os_printf("uart is coming! \r\n");
//	  if(pRxBuff->BuffState == EMPTY){
//		system_os_post(uart_recvTaskPrio, 0, 0);
//	  }else{
//		system_os_post(uart_recvTaskPrio, 2, 2);
//	  }

//	  if(0 > strcmp(pRxBuff->pRcvMsgBuff,"s")){
//		//INFO("uart received! \r\n");
//		system_os_post(uart_recvTaskPrio, SIG_UART0_RX, 20);
//	  }
	...
	//  }
	  if (UART_RXFIFO_FULL_INT_ST == (READ_PERI_REG(UART_INT_ST(uart_no)) & UART_RXFIFO_FULL_INT_ST))
	  {
	    ...
	    system_os_post(uart_recvTaskPrio, 3, c);
...
uart_recvTask(os_event_t *events)
{
	...
	static int i=0;
//	os_printf("recvtask is coming!!\n");
//	if(events->sig == SIG_UART0_RX){
//		if(events->par == 20)
//			os_printf("the par is 20!!!\n");
//	}
	if(events->sig == 3){
		//os_printf("the par is %c!!!\n",events->par);
		if((events->par != 0x0d) && (events->par != 0x0a)){
			data[i] = events->par;
			i++;
		}
		else if(events->par == 0x0d){
			data[i] = 0x0d;
			i++;
		}
		else if(events->par == 0x0a){
			data[i] = 0x0a;
			os_printf("uart i is %d\n", i);
			i=0;
			send_data(data);
		}
//		else{
...
//			os_printf("OKKKK\n");
//			send_data(data);
//			//system_os_post(1, mqtt_send, 1);
//		}
	}
	if(events->sig == 0){
		if(events->par == 0)
			os_printf("the par is 0!!!\n");
    #if  UART_BUFF_EN
        Uart_rx_buff_enq();
    #else
    ...
    #endif
    }else if(events->sig == 2){
    #if UART_BUFF_EN
	 //already move uart buffer output to uart empty interrupt
        //tx_start_uart_buffer(UART0);
    #else
//    	uart_tx_one_char(UART0, 6);
		os_printf("the par is is :&d!!!\n",events->par);
    #endif
    }
}
...
uart_init(UartBautRate uart0_br, UartBautRate uart1_br)
{
    ...
    #endif
    system_os_post(uart_recvTaskPrio, SIG_UART0_RX, 2);
}
